...
CHARACTER_STYLE_GUIDE = {
    "Harry Potter": (
        "MATCH ON: this is a NARROW, specific match, not a general "
        "'young person in glasses' bucket -- require ALL THREE of the "
        "following together, not just one or two: (1) glasses that are "
        "distinctly round/circular in shape, not oval, rectangular, or "
        "thick-framed, (2) visibly messy, unstyled, side-parted dark hair "
        "(not neat, not slicked, not light-colored), and (3) a genuinely "
        "wide-eyed, slightly startled or unsure look -- eyebrows a little "
        "raised, not a calm/neutral gaze. Male-presenting, young, "
        "no facial hair. IMPORTANT: round glasses alone, or round glasses "
        "plus an ordinary neutral/calm expression, is NOT enough -- that "
        "describes a huge number of ordinary photos and must NOT default "
        "here. If the hair is neat/styled rather than messy, or the "
        "expression is calm/composed rather than startled, do NOT pick "
        "this character no matter how good a glasses-based guess it seems "
        "-- look elsewhere on the roster instead (Sheldon for neat+prim, "
        "Professor for calm+lined). Do not choose this character out of "
        "familiarity or because no other entry seems to fit perfectly -- "
        "an imperfect but genuine match on another roster entry's MATCH ON "
        "criteria is always preferable to a default Harry Potter pick. "
    ...
    "Professor from Money Heist": (
        ...
        "forehead, a receding or thinning hairline, and a deep-set, "
        "genuinely unblinking steady gaze held with a completely still "
        "closed mouth. Male-presenting, clean-shaven. Glasses, if present, "
        "are rectangular or thick-rimmed -- never round (round glasses "
        "point to Harry Potter, not here). CRITICAL: glasses plus an "
        "ordinary neutral expression is common in regular photos and is "
        "NOT enough on its own -- only pick this character when the "
        "forehead lines, receding hairline, AND the unblinking steady gaze "
        "are all present together. If the face looks young (smooth "
        "forehead, full hairline), do not pick this character even if the "
        "person seems composed -- consider Sheldon or Batman instead. "
        "POSTER LOOK: default to the iconic heist-mode costume -- bright "
        "red jumpsuit plus a white Salvador Dali mask (painted grin, rosy "
        "cheeks, thin mustache) -- a bank vault or sunbaked Spanish plaza "
        "backdrop. Only use the alternate academic look (thick-rimmed "
        "glasses, tousled hair, rumpled cardigan) if the person's own "
        "glasses/cardigan were themselves the standout match reason."
    ),
    "Sheldon Cooper": (
        "MATCH ON: rectangular or thick-rimmed glasses specifically -- "
        "never round (round points to Harry Potter) -- thin lips pressed "
        "into a prim flat line with one corner very slightly higher than "
        "the other (faint superiority, not a real smile), and an exactly "
        "upright, rigid posture with arms crossed at precise angles. "
        "Male-presenting, clean-shaven, neat/styled hair (not messy -- "
        "messy hair points to Harry Potter, not here). CRITICAL: glasses "
        "plus a serious expression is common and NOT enough alone -- this "
        "needs the rectangular glasses shape AND the faint one-sided "
        "superior mouth AND the rigid exact posture all together. POSTER "
        "LOOK: side-swept hair, layered graphic tee under a collared "
        "shirt, cluttered apartment with an equation-covered whiteboard "
        "backdrop."
    ),
}
