...
CHARACTER_STYLE_GUIDE = {
    "Iron Man": (
        "MATCH ON: a sharply trimmed goatee or precisely lined stubble tracing "
        "the jawline, combined with one eyebrow noticeably raised higher than "
        "the other and a closed-mouth smirk pulling to one side. The goatee "
        "shape is the deciding cue here, not general 'confidence' -- a smirk "
        "without a defined goatee/stubble line is not enough on its own. "
        "Male-presenting, angular jaw, athletic-to-fit build rather than "
        "heavyset. POSTER LOOK: sleek red-and-gold powered armor, glowing "
    ...
    "Batman": (
        "MATCH ON: a broad, square jaw combined with a heavy brow that casts "
        "a slight shadow over the eyes, and lips pressed into a completely "
        "flat neutral line -- no frown, no smirk, no curve at all. "
        "Male-presenting, clean-shaven or very light stubble, broad "
        "shoulders/built frame. The square-jaw-plus-heavy-brow combination is "
        "what separates this from Jon Snow (has a beard, longer/wearier "
        "face), Sasuke (narrower, younger, thin straight brows), and Sheldon "
        "(prim closed-mouth smirk, not a flat line). POSTER LOOK: matte-black "
        "armored bat-suit, flowing cape, cowl with pointed ears, utility "
        "belt, moody Gotham-noir lighting (deep shadows, cold blue-grey "
        "highlights, rain-slicked rooftops)."
    ),
    "Spider-Man": (
        "MATCH ON: the mask would hide the entire face, so ignore facial "
        "features completely and match ONLY on body language: a clearly "
        "off-balance, mid-action pose (crouched low, one arm flung out, "
        "caught mid-jump) or an exaggerated bent-knee, bouncy stance. A "
        "standard still standing portrait -- even a cheerful one -- is NOT a "
        "match regardless of expression; there must be genuine dynamic "
        "motion in the pose. Slim-to-athletic build, younger-looking overall "
        "silhouette. POSTER LOOK: skin-tight red-and-blue spandex suit, "
        "black web patterning, large white reflective eye lenses, city "
        "skyline backdrop with web-lines."
    ),
    "Thomas Shelby": (
        "MATCH ON: high, sharp cheekbones on a lean face, thin straight-set "
        "lips held in total stillness (not even a flat neutral line -- "
        "genuinely motionless), and pale-to-neutral clean-shaven skin. "
        "Male-presenting, neat/tidy hair shape if visible. The sharp "
        "cheekbones plus total mouth stillness is the deciding combo -- "
        "distinct from Batman's squarer jaw and Sasuke's softer, younger "
        "skin. Posture is upright with hands clasped or still, not crossed."
    ),
    "Naruto": (
        "MATCH ON: a round, full-cheeked youthful face, a wide open-mouthed "
        "grin that shows the top row of teeth, and noticeably wide/bright "
        "eyes. Male-presenting, young-looking, no facial hair. The "
        "full-cheeked roundness plus visible-teeth grin is the key "
        "combination -- separates this from Shinchan (thicker, lower-set "
        "eyebrows and an even wider open-mouthed laugh) and Elle Woods "
        "(softer, closed-lipped brightness). Pose often mid-motion, arm "
        "raised or fist out. POSTER LOOK: orange-and-black jumpsuit, blue "
        "forehead protector, spiky blonde hair, whisker cheek markings, "
        "anime-style chakra-burst backdrop."
    ),
    "Professor from Money Heist": (
        "MATCH ON: a middle-aged face with visible fine lines on the "
        "forehead, a receding or thinning hairline, and a deep-set, genuinely "
        "unblinking steady gaze held with a completely still closed mouth. "
        "Male-presenting, clean-shaven. CRITICAL: glasses plus an ordinary "
        "neutral expression is common in regular photos and is NOT enough on "
        "its own -- only pick this character when the forehead lines, "
        "receding hairline, AND the unblinking steady gaze are all present "
        "together. If the face looks young (smooth forehead, full hairline), "
        "do not pick this character even if the person seems composed -- "
        "consider Sheldon or Batman instead. POSTER LOOK: default to the "
        "iconic heist-mode costume -- bright red jumpsuit plus a white "
        "Salvador Dali mask (painted grin, rosy cheeks, thin mustache) -- a "
        "bank vault or sunbaked Spanish plaza backdrop. Only use the "
        "alternate academic look (thick-rimmed glasses, tousled hair, "
        "rumpled cardigan) if the person's own glasses/cardigan were "
        "themselves the standout match reason."
    ),
    "Gwen Stacy": (
        "MATCH ON: an angular, slightly pointed chin, one eyebrow cocked "
        "distinctly higher than the other paired with a closed-mouth smirk "
        "pulling to one corner only, and a loose, casual head tilt. "
        "Female-presenting. The pointed chin plus single raised eyebrow plus "
        "head tilt together is the differentiator -- separates this from "
        "Regina George (smirk is even on both sides, no head tilt, chin held "
        "level or down) and Black Widow (no smirk at all, totally flat "
        "mouth). POSTER LOOK: pink-and-white hooded Spider suit with a black "
        "spider emblem, hood down to show blonde-and-pink-streaked hair, "
        "comic-book graffiti backdrop."
    ),
    "Black Widow": (
        "MATCH ON: a level, direct gaze with a completely flat mouth -- zero "
        "smile or smirk, not even a one-sided curve -- combined with sharp, "
        "angular cheekbones and an upright neck/shoulder line with no head "
        "tilt at all. Female-presenting, toned/athletic build. The fully "
        "flat mouth plus straight neck (no tilt) is what separates this from "
        "Gwen Stacy's smirk-and-tilt. Auburn or reddish hair tone is a bonus "
        "cue if visible, not required. POSTER LOOK: fitted black tactical "
        "bodysuit, utility straps, wavy red hair, cool steel-blue or "
        "shadowed tactical backdrop."
    ),
    "Daenerys Targaryen": (
        "MATCH ON: a wide, high forehead, softly rounded cheeks (not sharp "
        "or angular), calm wide-set eyes, and a chin held level and very "
        "slightly lifted -- not tilted to a side, not dropped down. "
        "Female-presenting; pale or light hair is a bonus cue if visible but "
        "not required. The wide forehead plus soft rounded cheeks separates "
        "this from Black Widow's sharp cheekbones and Hermione's furrowed "
        "brow. POSTER LOOK: long silver-platinum braided hair, flowing pale "
        "ivory or deep-blue gown, sunbaked desert city or grand throne-hall "
        "backdrop."
    ),
    "Hermione Granger": (
        "MATCH ON: visibly furrowed or knit-together eyebrows (active "
        "concentration, not resting), wide alert eyes tracking forward, and "
        "an oval face shape, often with glasses or hair neatly pulled back. "
        "Female-presenting. The furrowed brow plus alert forward gaze is the "
        "differentiator -- separates this from Wednesday's perfectly flat, "
        "unmoving brow and Daenerys's smooth, unfurrowed forehead. POSTER "
        "LOOK: bushy brown hair, black Gryffindor robes with a "
        "maroon-and-gold tie, candlelit castle library backdrop."
    ),
    "Wednesday Addams": (
        "MATCH ON: perfectly flat, unmoving eyebrows (no furrow, no arch), "
        "a genuinely unblinking wide stare, and thin closed lips with "
        "literally zero curve in either direction -- not even the faintest "
        "smirk. Female-presenting, dark or neutral-toned hair if visible. "
        "This must read as more extreme than an ordinary neutral photo face: "
        "eyebrows flat (not furrowed like Hermione, not arched like Gwen), "
        "mouth with zero curve (not a one-sided smirk like Gwen or Regina). "
        "POSTER LOOK: jet-black hair in two tight braids, black "
        "Victorian-collared dress, foggy graveyard or gothic-academy "
        "backdrop."
    ),
    "Kakashi Hatake": (
        "MATCH ON: one eyelid visibly more closed or drooping than the "
        "other (a genuinely asymmetrical, sleepy-eyed look), a fully "
        "closed-mouth ease with no visible teeth, and slightly uneven or "
        "tousled hair if visible. Male-presenting. CRITICAL: an ordinary "
        "relaxed face or a generic half-smile is NOT enough on its own -- "
        "this requires the specific asymmetrical one-eye-more-closed detail. "
        "If both eyes are evenly open, do NOT pick this character even if "
        "the person looks laid-back -- consider Iron Man's one-sided smirk "
        "or Gwen's cocked eyebrow instead. This should be a rare, "
        "specific match, not a fallback for 'chill-looking' photos. POSTER "
        "LOOK: messy silver hair, Leaf Village forehead protector tilted "
        "over one eye, fabric mask over nose and mouth, navy flak vest, "
        "misty forest or rooftop-at-dusk backdrop."
    ),
    "Sasuke Uchiha": (
        "MATCH ON: a young, narrow/angular face with thin, straight "
        "eyebrows pulled slightly inward (not arched), narrow or hooded eyes "
        "fixed in a hard stare, and completely flat, closed lips. "
        "Male-presenting, young, dark hair if visible, no facial hair. The "
        "straight-inward eyebrows plus narrow-eye stare together separate "
        "this from Batman's heavier brow/squarer jaw and from Jon Snow's "
        "older, bearded, more weathered look. POSTER LOOK: navy-and-white "
        "high-collared ninja outfit, katana on the back, Sharingan eye "
        "detail, crackling blue lightning backdrop."
    ),
    "Joker": (
        "MATCH ON: a wide grin that shows both teeth AND gums (more "
        "exaggerated than a normal happy smile), eyebrows raised unusually "
        "high on the forehead, and wide, slightly bulging eyes. "
        "Male-presenting. Loose, theatrical hand or head gestures rather "
        "than a still pose. The teeth-and-gums grin plus unusually high "
        "raised eyebrows is what separates this from Naruto's warmer, more "
        "closed-cheek grin and from Shinchan's lower, thicker eyebrows. "
        "POSTER LOOK: rumpled purple tailcoat over a green vest, "
        "chalk-white face paint, smeared red smile, chaotic neon-lit city "
        "backdrop."
    ),
    "Jon Snow": (
        "MATCH ON: visible beard or heavy stubble covering the jaw and "
        "chin, a long face shape, downturned or sunken-looking eyes, and a "
        "heavy single-line brow furrow. Male-presenting. Visible "
        "beard/stubble is close to a hard requirement here -- it's what "
        "rules out clean-shaven Batman and clean-shaven Sasuke. Do not pick "
        "this character on a clean-shaven face even if the expression seems "
        "weary or serious. POSTER LOOK: dark tousled shoulder-length hair, "
        "thick black furs and leather armor, snow-covered northern "
        "landscape or icy-wall backdrop."
    ),
    "Shinchan": (
        "MATCH ON: thick, low-set eyebrows drawn close down near the eyes, "
        "an exaggeratedly wide open-mouthed laugh (wider than a simple "
        "smile), and a flat, simple round face. Reads as cartoonishly goofy "
        "rather than genuinely warm. The thick low eyebrows plus the "
        "wide-open laughing mouth (not just a grin) separates this from "
        "Naruto's fuller-cheeked grin and Joker's manic teeth-and-gums grin "
        "-- Shinchan reads silly/innocent, not warm or unhinged. POSTER "
        "LOOK: spiky messy black hair, simple bright shirt and shorts, bold "
        "flat colorful cartoon-style backdrop."
    ),
    "Harry Potter": (
        "MATCH ON: round-SHAPED glasses specifically (circular lenses, not "
        "rectangular or thick-rimmed), a round-ish youthful face, messy or "
        "unkempt hair if visible, and wide eyes that read as earnest/a "
        "little unsure rather than confident or flat. Male-presenting, "
        "young, no facial hair. CRITICAL: glasses alone are not enough -- "
        "they must specifically read as round-shaped AND be paired with the "
        "youthful, uncertain look. Rectangular/thick-rimmed glasses point to "
        "Sheldon or the Professor instead, not here. POSTER LOOK: round "
        "glasses, lightning-bolt scar, messy black hair, Gryffindor robes "
        "with a red-and-gold scarf, Hogwarts castle or starry quidditch-pitch "
        "backdrop."
    ),
    "Dr. Doom": (
        "MATCH ON: the mask/armor would hide the face entirely, so match "
        "ONLY on posture, never on any facial cue: arms crossed firmly over "
        "the chest or held rigidly at the sides, chin tilted upward, "
        "shoulders squared and still, a statuesque commanding stance rather "
        "than a relaxed one. Works for either gender presentation. POSTER "
        "LOOK: full green-and-metal armor, smooth expressionless mask, "
        "flowing green hooded cloak, gothic stone castle or "
        "arcane-green-energy backdrop."
    ),
    "Regina George": (
        "MATCH ON: sleek, smoothed-down hair (not tousled or messy), a "
        "closed-mouth smirk that is EVEN on both sides (unlike a one-sided "
        "smirk), and sharp defined cheekbones held with the chin very "
        "slightly down in a 'looking down on you' angle. Female-presenting. "
        "The even two-sided smirk plus chin-down angle is what separates "
        "this from Gwen Stacy's one-sided grin-and-head-tilt. POSTER LOOK: "
        "sleek blonde hair, trendy pink or designer outfit, glossy "
        "high-school-hallway or pastel-pink backdrop."
    ),
    "Elle Woods": (
        "MATCH ON: a wide, fully open-mouthed smile showing both rows of "
        "teeth, big rounded eyes, and a soft, rounded face shape (not sharp "
        "cheekbones). Female-presenting. The full-teeth open smile plus "
        "rounded soft face separates this from Regina's closed even smirk; "
        "against Naruto's similarly open grin, lean on an extra bright, "
        "bouncy pose (hands clasped excitedly, slight forward bounce) as the "
        "tiebreaker. POSTER LOOK: voluminous blonde hair, bold pink outfit "
        "(blazer, dress, or accessories), sunny pastel-campus or courtroom "
        "backdrop."
    ),
    "Sheldon Cooper": (
        "MATCH ON: rectangular or thick-rimmed glasses (distinct shape from "
        "Harry Potter's round lenses), thin lips pressed into a prim flat "
        "line with one corner very slightly higher than the other (faint "
        "superiority, not a real smile), and an exactly upright, rigid "
        "posture with arms crossed at precise angles. Male-presenting, "
        "clean-shaven. CRITICAL: glasses plus a serious expression is common "
        "and NOT enough alone -- this needs the rectangular glasses shape "
        "AND the faint one-sided superior mouth AND the rigid exact posture "
        "all together. POSTER LOOK: side-swept hair, layered graphic tee "
        "under a collared shirt, cluttered apartment with an "
        "equation-covered whiteboard backdrop."
    ),
}
