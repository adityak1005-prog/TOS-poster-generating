...
CHARACTER_STYLE_GUIDE = {
    "Iron Man": (
        "MATCH ON: a sharply trimmed goatee or precisely lined stubble "
        "along the jaw, ONE eyebrow raised distinctly higher than the "
        "other, and a closed-mouth smirk pulling to one side only. "
        "Male-presenting, angular jaw. REQUIRES visible facial hair in a "
        "goatee/lined shape -- without it, do not pick this even if the "
        "smirk and raised brow are present (see Sheldon for a similar "
        "faint one-sided mouth but clean-shaven). POSTER LOOK: sleek "
        "red-and-gold powered armor, glowing blue arc reactor at the "
        "chest, faceplate open to reveal the goatee underneath, repulsor "
        "palms raised, bright high-tech backdrop (lab light, sky, "
        "explosion glow)."
    ),
    "Batman": (
        "MATCH ON: a broad SQUARE jaw, a heavy brow shadowing the eyes, "
        "and lips pressed into a completely flat line with zero curve in "
        "either direction. Male-presenting, clean-shaven, broad build. "
        "The jaw must read as square/wide, not narrow -- a narrow or "
        "angular jaw with the same flat mouth points to Sasuke instead. "
        "No beard (a bearded version of this look is Jon Snow). POSTER "
        "LOOK: matte-black armored bat-suit, flowing cape, cowl with "
        "pointed ears, utility belt, moody Gotham-noir lighting (deep "
        "shadows, cold blue-grey highlights, rain-slicked rooftops)."
    ),
    "Spider-Man": (
        "MATCH ON: mask hides the face entirely -- match ONLY on body "
        "language, never expression. Requires genuine mid-action motion: "
        "crouched low, one arm flung out, caught off-balance mid-jump. A "
        "standard still standing photo, however energetic the face looks, "
        "does NOT qualify -- there must be actual dynamic motion in the "
        "limbs/posture. Slim-to-athletic, younger-looking build. POSTER "
        "LOOK: skin-tight red-and-blue spandex suit, black web patterning, "
        "large white reflective eye lenses, city skyline backdrop with "
        "web-lines."
    ),
    "Thomas Shelby": (
        "MATCH ON: high, sharp cheekbones on a LEAN (not square, not "
        "round) face, and thin lips held in genuine total stillness -- "
        "more still than a simple flat line, no micro-movement at all. "
        "Male-presenting, clean-shaven, neat hair. The sharp cheekbones "
        "on a lean face is the deciding shape cue -- Batman's jaw is "
        "square/broad instead, Sasuke's face is younger and narrower "
        "still. Upright posture, hands still or clasped, not crossed."
    ),
    "Naruto": (
        "MATCH ON: a ROUND, full-cheeked youthful face AND a wide "
        "open-mouth grin showing the top teeth. Both the cheek roundness "
        "and the visible teeth are required together -- a grin on a "
        "narrower face points elsewhere. Male-presenting, young, no "
        "facial hair. Against Joker: no raised/arched eyebrows and no gum "
        "exposure, just a warm full-cheeked grin. Against Shinchan: "
        "eyebrows are relaxed/normal height, not thick and pulled low. "
    ...
    "Professor from Money Heist": (
        "MATCH ON: a middle-aged face with visible forehead lines, a "
        "receding or thinning hairline, and a deep-set, genuinely "
        "unblinking gaze with a completely still closed mouth. "
        "Male-presenting, clean-shaven. If glasses are present they read "
        "as rectangular/thick, never round. REQUIRES the aged "
        "signals (forehead lines + receding hairline) together with the "
        "steady gaze -- a young smooth-foreheaded face does not qualify "
        "no matter how composed it looks; that points to Sheldon or "
        "Batman instead. POSTER LOOK: default to the iconic heist-mode "
    ...
    "Gwen Stacy": (
        "MATCH ON: a pointed/angular chin, ONE eyebrow cocked distinctly "
        "higher than the other, a one-sided closed-mouth smirk, AND a "
        "loose casual head tilt -- all four together. Female-presenting. "
        "The head tilt is what separates this from Regina George, whose "
        "smirk is even on both sides with the chin held level or down, "
        "not tilted. POSTER LOOK: pink-and-white hooded Spider suit with "
        "a black spider emblem, hood down to show "
        "blonde-and-pink-streaked hair, comic-book graffiti backdrop."
    ),
    "Black Widow": (
        "MATCH ON: a completely flat mouth (zero curve, not even "
        "one-sided), sharp angular cheekbones, and a straight, level "
        "neck/shoulder line with NO head tilt. Female-presenting, toned "
        "build. The absence of any tilt plus the fully flat mouth (not a "
        "smirk at all) separates this from Gwen Stacy. Reddish hair is a "
        "bonus cue, not required. POSTER LOOK: fitted black tactical "
        "bodysuit, utility straps, wavy red hair, cool steel-blue or "
        "shadowed tactical backdrop."
    ),
    "Daenerys Targaryen": (
        "MATCH ON: a wide, high forehead, softly ROUNDED cheeks (not "
        "sharp/angular), and a chin held level and very slightly raised "
        "-- not tilted sideways, not dropped. Female-presenting, "
        "light/neutral hair. The rounded-cheek softness plus level-raised "
        "chin (no side tilt) separates this from Black Widow's sharp "
        "cheekbones and Gwen's tilted head. POSTER LOOK: long "
        "silver-platinum braided hair, flowing pale ivory or deep-blue "
        "gown, sunbaked desert city or grand throne-hall backdrop."
    ),
    "Hermione Granger": (
        "MATCH ON: eyebrows visibly KNIT TOGETHER in active concentration "
        "(not resting flat, not arched), combined with wide, alert eyes "
        "tracking forward and an oval face. Female-presenting, hair often "
        "pulled back or a bit bushy. The knit/furrowed brow is the key -- "
        "distinct from Wednesday's perfectly flat unmoving brow and from "
        "Daenerys's smooth unfurrowed forehead. POSTER LOOK: bushy brown "
        "hair, black Gryffindor robes with a maroon-and-gold tie, "
        "candlelit castle library backdrop."
    ),
    "Wednesday Addams": (
        "MATCH ON: eyebrows completely FLAT and unmoving (passive, not "
        "furrowed like Hermione, not arched like Gwen), a genuinely "
        "unblinking wide stare, and lips with ZERO curve in either "
        "direction -- flatter than Black Widow's flat mouth, with no "
        "engagement in the eyes at all, reading as bored/checked-out "
        "rather than alert. Female-presenting, dark/neutral hair. POSTER "
        "LOOK: jet-black hair in two tight braids, black "
        "Victorian-collared dress, foggy graveyard or gothic-academy "
        "backdrop."
    ),
    "Kakashi Hatake": (
        "MATCH ON: ONE eyelid visibly more closed/drooping than the "
        "other -- a genuine physical asymmetry between the two eyes, not "
        "just a relaxed look -- paired with a fully closed, closed-mouth "
        "calm. Male-presenting. This is a RARE, specific physical trait: "
        "if both eyes are evenly open, this is NOT a match no matter how "
        "laid-back the person looks -- that points to Iron Man's "
        "one-sided smirk or Thomas Shelby's stillness instead. Do not use "
        "this as a catch-all for 'chill-looking' photos. POSTER LOOK: "
        "messy silver hair, Leaf Village forehead protector tilted over "
        "one eye, fabric mask over nose and mouth, navy flak vest, misty "
        "forest or rooftop-at-dusk backdrop."
    ),
    "Sasuke Uchiha": (
        "MATCH ON: a young, NARROW/angular face (not square like Batman, "
        "not round like Naruto), thin straight eyebrows pulled slightly "
        "inward, narrow or hooded eyes in a hard stare, and flat closed "
        "lips. Male-presenting, young, no facial hair. The narrow face "
        "shape plus inward-pulled straight brows is the differentiator "
        "from Batman (square jaw, heavier brow) and Jon Snow (older, "
        "bearded). POSTER LOOK: navy-and-white high-collared ninja "
        "outfit, katana on the back, Sharingan eye detail, crackling blue "
        "lightning backdrop."
    ),
    "Joker": (
        "MATCH ON: a wide grin showing BOTH teeth and gums (more extreme "
        "than a normal smile), eyebrows raised UNUSUALLY high, and wide, "
        "slightly bulging eyes. Male-presenting. The gum exposure plus "
        "unusually high eyebrows is required together -- separates this "
        "from Naruto's warmer teeth-only grin with relaxed brows, and "
        "from Shinchan's thick LOW-set brows. POSTER LOOK: rumpled purple "
        "tailcoat over a green vest, chalk-white face paint, smeared red "
        "smile, chaotic neon-lit city backdrop."
    ),
    "Jon Snow": (
        "MATCH ON: visible beard or heavy stubble covering the jaw/chin "
        "(near-mandatory -- do not pick this clean-shaven), a long face "
        "shape, and downturned or heavy-lidded eyes with a single-line "
        "brow furrow. Male-presenting. The beard is what rules out "
        "Batman and Sasuke, who are both clean-shaven. POSTER LOOK: dark "
        "tousled shoulder-length hair, thick black furs and leather "
        "armor, snow-covered northern landscape or icy-wall backdrop."
    ),
    "Shinchan": (
        "MATCH ON: thick, LOW-set eyebrows drawn down close to the eyes, "
        "combined with an exaggeratedly wide OPEN-mouthed laugh (mouth "
        "wide open, not just a grin), and a flat round face. The "
        "low-set-thick-brows is the key differentiator from Joker's "
        "high-arched brows and Naruto's relaxed brows. POSTER LOOK: spiky "
        "messy black hair, simple bright shirt and shorts, bold flat "
        "colorful cartoon-style backdrop."
    ),
    "Harry Potter": (
        "MATCH ON: this is a rare, narrow match -- require ALL THREE "
        "together: (1) ROUND/circular-shaped glasses specifically, not "
        "oval or rectangular, (2) visibly messy, unstyled dark hair, and "
        "(3) a wide-eyed, startled or unsure look with eyebrows a little "
        "raised -- not a calm or neutral gaze. Male-presenting, young. "
        "Round glasses alone, or round glasses plus a calm expression, is "
        "NOT enough -- that describes too many ordinary photos and must "
        "not default here. Neat/styled hair or a calm expression rules "
        "this out even with round glasses present -- go to Sheldon (neat, "
        "rectangular glasses, prim) or the Professor (calm, aged) "
        "instead. Do not choose this out of familiarity when nothing "
        "fits perfectly -- an imperfect genuine match elsewhere beats a "
        "default pick here. POSTER LOOK: round glasses, lightning-bolt "
        "scar, messy black hair, Gryffindor robes with a red-and-gold "
        "scarf, Hogwarts castle or starry quidditch-pitch backdrop."
    ),
    "Dr. Doom": (
        "MATCH ON: mask/armor hides the face -- match ONLY on posture: "
        "arms crossed firmly or held rigidly at the sides, chin tilted "
        "upward, shoulders squared, a statuesque commanding stance held "
        "completely still (no dynamic motion -- that's Spider-Man's "
        "cue instead). Works for either gender presentation. POSTER "
    ...
    "Regina George": (
        "MATCH ON: sleek, smoothed-down hair, a closed-mouth smirk that "
        "is EVEN on both sides (not one-sided), sharp cheekbones, and the "
        "chin held very slightly DOWN in a looking-down-on-you angle. "
        "Female-presenting. The even two-sided smirk plus chin-down angle "
        "(no tilt) separates this from Gwen Stacy's one-sided "
        "smirk-plus-head-tilt. POSTER LOOK: sleek blonde hair, trendy "
        "pink or designer outfit, glossy high-school-hallway or "
        "pastel-pink backdrop."
    ),
    "Elle Woods": (
        "MATCH ON: a full OPEN-mouthed smile showing both rows of teeth, "
        "big rounded eyes, a soft rounded face (not sharp cheekbones), "
        "AND a bouncy, forward-leaning energetic pose. Female-presenting. "
        "Against Regina: mouth is open/full, not a closed even smirk. "
        "Against Naruto: female-presenting with a softer, rounder face "
        "and an extra bouncy/clasped-hands pose as the tiebreak. POSTER "
        "LOOK: voluminous blonde hair, bold pink outfit (blazer, dress, "
        "or accessories), sunny pastel-campus or courtroom backdrop."
    ),
    "Sheldon Cooper": (
        "MATCH ON: RECTANGULAR or thick-rimmed glasses (never round -- "
        "round points to Harry Potter), thin lips in a prim flat line "
        "with one corner very slightly higher (faint superiority, not a "
        "real smile), an exactly upright rigid posture with arms crossed "
        "at precise angles, and neat/styled hair (not messy). "
        "Male-presenting, clean-shaven. Requires the glasses shape AND "
        "the faint one-sided prim mouth AND the rigid posture together -- "
        "any one alone is not enough. POSTER LOOK: side-swept hair, "
        "layered graphic tee under a collared shirt, cluttered apartment "
        "with an equation-covered whiteboard backdrop."
    ),
}
